...
<div class="box">
    <div class="box-header">
        <h5 class="box-title" style="text-align:center; color:dark-blue"><b>Legalitas tanah objek ({{$tanah->kerjainternals->peruntukan}}) lokasi di {{$tanah->kerjainternals->alamat}}</b></h5><br>
       
    </div>
</div>
   <div class="box-body">
    <form action="{{ route('tanah.updateinternal', $tanah)}}" method="POST">
        @csrf
        @method("PUT")
        <div class="row">   
             
            <div class="input-field col s12">
                <input disabled value="{{number_format($tanah->kerjainternals->nilai_pasar)}}" id="disabled"
                type="text" class="validate">
                <label for="disabled">Nilai Pasar Objek</label>
            </div>

            <div class="input-field col s12">
                <input disabled value="{{number_format($tanah->kerjainternals->nilai_likuidasi)}}" id="disabled"
                type="text" class="validate">
                <label for="disabled">Nilai Likuidasi Objek</label>
            </div>

            <div class="input-field col s12">
                <input disabled value="{{number_format($tanah->kerjainternals->luas_tanah)}}" id="disabled"
                type="text" class="validate">
                <label for="disabled">Luas Tanah Total</label>
            </div>
