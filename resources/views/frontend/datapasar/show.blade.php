...
<div class="box">
    <div class="box-header">
        <h5 class="box-title" style="text-align:left;"><b>Data Pembanding Objek {{$kerjareviews->first()->nama_debitur}}</b></h5>  
            ...
            <form action="{{route ('datapasar.create')}}" method="POST">
                ...
                <select name="alamat"  class="browser-default">
                    @foreach ($kerjareviews as $kerjareview)
                     <option value="{{$kerjareview->alamat }}">{{$kerjareview->alamat}}</option>
                    @endforeach
                </select> 
